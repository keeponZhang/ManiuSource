 ...
 * 返回值类型为int。
 */
//    func2  本质 数组，数组里面存放的是指针，每一个指针是函数
 ...
 * 右边是一个[]运算符号，说明func是一个指向数组的指针，
  现在往左看，左边有一个*号，说明这个数组的元素是指针，
    */
    //func3是一个指针，指向了一个数组，数组的元素是指针，指针指向的是函数
    //(*func3)[5] func3是一个数组指针，指向一个数组
    //*(*func3)[5] 说明数组元素也是一个指针
    //int (*(*func3)[5])(int *pInt)指向的是一个函数
...
//*func4是一个指针
//(*func4)(int *pInt) 指向的是一个函数，函数参数是int *
//(*(*func4)(int *pInt)) ，代表函数返回值是一个指针
//(*(*func4)(int *pInt))[5] ，代表函数返回值是一个指针，指针指向的是一个数组
// int (*(*func4)(int *pInt))[5] ，代表函数返回值是一个指针，指针指向的是一个数组，数组存的是int
...
int*(*(*pfun)(int *))[10];
//1.pfun遇到了*，说明pfun是一个指针（再往左看又遇到了括号，因此又要调转方向）
//2.遇到的是另外一个括号，说明指针所指向的是一个函数，函数的参数是一个整型指针
//3.然后又向相反的方向看，又遇到了一个*，说明该函数的返回值是一个指针（再往左看又遇到括号，然后再次调转方向，把内侧的括号里的内容看完，出了括号遇到的是数组）
//4.说明指针指向的函数的返回值类型的指针指向的是数组
